...
impl Range {
    ...
    pub fn remove(self, other: Self) -> Vec<Self> {
        todo!();
    }
    ...
    pub fn difference(self, other: Self) -> Vec<Self> {
        ...
        if other.start > self.start && other.end < self.end {
            return vec![Range::new(self.start, other.start).unwrap(),
            Range::new(other.end, self.end).unwrap(),
            ];
        }

        // special case 4: left side gone
        if other.contains(self.start) {
            return vec![Range::new(other.end, self.end).unwrap()];
        }
...
mod test {
    ...
    fn intersection() {
        ...
        assert_eq!(ten_to_15.intersection(nine_ten), Some(Range::new(10, 11).unwrap()));

        // the range 9, 10, 11
        let nine_to_11 = Range::new(9, 12).unwrap();
        assert_eq!(ten_to_15.intersection(nine_to_11), Some(Range::new(10, 12).unwrap()));

        // the range 10
        let ten = Range::new(10, 11).unwrap();
        assert_eq!(ten_to_15.intersection(ten), Some(Range::new(10, 11).unwrap()));

        // the range 10, 11
        let ten_to_11 = Range::new(10, 12).unwrap();
        assert_eq!(ten_to_15.intersection(ten_to_11), Some(Range::new(10, 12).unwrap()));

        // the range 10, 11, 12, 13, 14
        let ten_to_14 = Range::new(10, 15).unwrap();
        assert_eq!(ten_to_15.intersection(ten_to_14), Some(Range::new(10, 15).unwrap()));

        // the range 11, 12, 13, 14
        let eleven_to_14 = Range::new(11, 15).unwrap();
        assert_eq!(ten_to_15.intersection(eleven_to_14), Some(Range::new(11, 15).unwrap()));

        // the range 10, 11, 12, 13, 14, 15
        assert_eq!(ten_to_15.intersection(ten_to_15), Some(Range::new(10, 16).unwrap()));

        // the range 10, 11, 12, 13, 14, 15, 16
        let ten_to_16 = Range::new(10, 17).unwrap();
        assert_eq!(ten_to_15.intersection(ten_to_16), Some(Range::new(10, 16).unwrap()));

        // the range 9, 10, 11, 12, 13, 14, 15, 16
        let nine_to_16 = Range::new(9, 17).unwrap();
        assert_eq!(ten_to_15.intersection(nine_to_16), Some(Range::new(10, 16).unwrap()));

        // the range 14, 15, 16
        let fourteen_to_16 = Range::new(14, 17).unwrap();
        assert_eq!(ten_to_15.intersection(fourteen_to_16), Some(Range::new(14, 16).unwrap()));

        // the range 15, 16
        let fifteen_to_16 = Range::new(15, 17).unwrap();
        assert_eq!(ten_to_15.intersection(fifteen_to_16), Some(Range::new(15, 16).unwrap()));

        // the range 15
        let fifteen = Range::new(15, 16).unwrap();
        assert_eq!(ten_to_15.intersection(fifteen), Some(Range::new(15, 16).unwrap()));
    ...
    fn difference() {
        ...
        // the range 1, 2, 3
        let one_to_3 = Range::new(1, 4).unwrap();
        assert_eq!(five_to_nine.difference(one_to_3), vec![Range::new(5, 10).unwrap()]);

        // the range 1, 2, 3, 4
        let one_to_4 = Range::new(1, 5).unwrap();
        assert_eq!(five_to_nine.difference(one_to_4), vec![Range::new(5, 10).unwrap()]);

        // the range 1, 2, 3, 4, 5
        let one_to_5 = Range::new(1, 6).unwrap();
        assert_eq!(five_to_nine.difference(one_to_5), vec![Range::new(6, 10).unwrap()]);

        // the range 1, 2, 3, 4, 5, 6
        let one_to_6 = Range::new(1, 7).unwrap();
        assert_eq!(five_to_nine.difference(one_to_6), vec![Range::new(7, 10).unwrap()]);

        // the range 1, 2, 3, 4, 5, 6, 7, 8
        let one_to_8 = Range::new(1, 9).unwrap();
        assert_eq!(five_to_nine.difference(one_to_8), vec![Range::new(9, 10).unwrap()]);
        ...
        assert_eq!(five_to_nine.difference(six_to_9), vec![Range::new(5, 6).unwrap()]);

        // the range 7, 8, 9
        let seven_to_9 = Range::new(7, 10).unwrap();
        assert_eq!(five_to_nine.difference(seven_to_9), vec![Range::new(5, 7).unwrap()]);

        // the range 7, 8, 9, 10
        let seven_to_10 = Range::new(7, 11).unwrap();
        assert_eq!(five_to_nine.difference(seven_to_10), vec![Range::new(5, 7).unwrap()]);

        // the range 9, 10
        let nine_ten = Range::new(9, 11).unwrap();
        assert_eq!(five_to_nine.difference(nine_ten), vec![Range::new(5, 9).unwrap()]);

        // the range 9
        let nine = Range::new(9, 10).unwrap();
        assert_eq!(five_to_nine.difference(nine), vec![Range::new(5, 9).unwrap()]);

        // the range 10
        let ten = Range::new(10, 11).unwrap();
        assert_eq!(five_to_nine.difference(ten), vec![Range::new(5, 10).unwrap()]);

        // the range 6, 7, 8
        let six_to_8 = Range::new(6, 9).unwrap();
        assert_eq!(five_to_nine.difference(six_to_8), vec![Range::new(5, 6).unwrap(), Range::new(9, 10).unwrap()]);

        // the range 7
        let seven = Range::new(7, 8).unwrap();
        assert_eq!(five_to_nine.difference(seven), vec![Range::new(5, 7).unwrap(), Range::new(8, 10).unwrap()]);
    }
}